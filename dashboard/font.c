...
// Define the font data. This file contains the single definition of the
// `ASCII` array. The header `font.h` contains only an `extern` declaration.
//
// Mapping notes:
// - The array index corresponds to the byte value used by `lcd_char()`.
// - Indices 0x00..0x1F are control/non-printable characters (kept as 0s).
// - Printable ASCII characters are at indices 0x20..0x7E (space..~).
//   Example mapping (hex -> char):
...
//   0x7B '{'  0x7C '|'  0x7D '}'  0x7E '~'
//
// - The code in `lcd.c` handles UTF-8 Cyrillic sequences and maps them into
//   the extended indices starting at 0x80 (e.g. Cyrillic А..я mapped to
//   ASCII[0x80..0xFF] according to the mapping in `lcd.c`). If you need to
//   inspect a particular glyph, compute its byte value and look it up by
//   index in this array.
//
// If you prefer, I can annotate each array row with an inline comment like
//   /* 0x41 'A' */ next to the initializer. Tell me if you want full-line
// annotations (verbose) or only the printable range.
...
const unsigned char ASCII[][5] = {
   ...
   { 0x7E, 0x11, 0x11, 0x11, 0x7E },   // А 0x80 128 ++
   { 0x7F, 0x49, 0x49, 0x49, 0x30 },   // Б 0x81 129 ++
   { 0x7F, 0x49, 0x49, 0x49, 0x36 },   // В 0x82 130 ++
   { 0x7F, 0x01, 0x01, 0x01, 0x01 },   // Г 0x83 131 ++
   { 0x60, 0x3E, 0x21, 0x3E, 0x60 },   // Д 0x84 132 ++
   { 0x7F, 0x49, 0x49, 0x49, 0x41 },   // Е 0x85 133 ++
   { 0x7F, 0x08, 0x7F, 0x08, 0x7F },   // Ж 0x86 134 ++
   { 0x22, 0x49, 0x49, 0x49, 0x36 },   // З 0x87 135 ++
   { 0x7F, 0x10, 0x08, 0x04, 0x7F },   // И 0x88 136 ++
   { 0x7F, 0x20, 0x11, 0x08, 0x7F },   // Й 0x89 137 ++
   { 0x7F, 0x08, 0x14, 0x22, 0x41 },   // К 0x8A 138 ++
   { 0x40, 0x3E, 0x01, 0x01, 0x7F },   // Л 0x8B 139
   { 0x7F, 0x02, 0x04, 0x02, 0x7F },   // М 0x8C 140
   { 0x7F, 0x08, 0x08, 0x08, 0x7F },   // Н 0x8D 141 ++
   { 0x3E, 0x41, 0x41, 0x41, 0x3E },   // О 0x8E 142 ++
   { 0x7F, 0x01, 0x01, 0x01, 0x7F },   // П 0x8F 143 ++
   { 0x7F, 0x09, 0x09, 0x09, 0x06 },   // Р 0x90 144 ++
   { 0x3E, 0x41, 0x41, 0x41, 0x22 },   // С 0x91 145 ++
   { 0x01, 0x01, 0x7F, 0x01, 0x01 },   // Т 0x92 146 ++
   { 0x47, 0x48, 0x48, 0x48, 0x3F },   // У 0x93 147 ++ 
   { 0x0E, 0x11, 0x7F, 0x11, 0x0E },   // Ф 0x94 148 ++
   { 0x63, 0x14, 0x08, 0x14, 0x63 },   // Х 0x95 149 ++
   { 0x3F, 0x20, 0x20, 0x20, 0x7F },   // Ц 0x96 150 ++
   { 0x0F, 0x08, 0x08, 0x08, 0x7F },   // Ч 0x97 151 ++
   { 0x7F, 0x40, 0x7F, 0x40, 0x7F },   // Ш 0x98 152 ++
   { 0x3F, 0x20, 0x3F, 0x20, 0x7F },   // Щ 0x99 153 ++
   { 0x01, 0x7F, 0x48, 0x48, 0x30 },   // Ъ 0x9A 154 ++
   { 0x7F, 0x48, 0x48, 0x30, 0x7F },   // Ы 0x9B 155 ++
   { 0x7F, 0x48, 0x48, 0x48, 0x30 },   // Ь 0x9C 156 ++
   { 0x41, 0x49, 0x49, 0x49, 0x3E },   // Э 0x9D 157 ++
   { 0x7F, 0x08, 0x3E, 0x41, 0x3E },   // Ю 0x9E 158 ++
   { 0x46, 0x29, 0x19, 0x09, 0x7F },   // Я 0x9F 159 ++
   ...
   { 0x7E, 0x4B, 0x4A, 0x4B, 0x4A },   // Ё 0xF0 240 ++
